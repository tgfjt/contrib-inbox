...
use gpui::{
    App, Context, Entity, FocusHandle, Focusable, IntoElement, KeyBinding, Render, Styled,
    Window, actions, div, prelude::*, px, rgb,
};
use time::{OffsetDateTime, format_description::well_known::Rfc3339};

use crate::github::{self, CheckRuns, Comment, Item, Review};
...
impl Inbox {
    ...
    fn boot(&mut self, cx: &mut Context<Self>) {
        if self.login.is_none() {
            ...
            cx.spawn(async move |_, cx| {
                ...
                this.update(cx, |inbox: &mut Inbox, cx| {
                    match login {
                        Ok(login) => {
    ...
    // ---- commands (called from key/click handlers) ----

    pub fn do_refresh(&mut self, cx: &mut Context<Self>) {
        let (token, login) = match (self.token.clone(), self.login.clone()) {
            (Some(token), Some(login)) => (token, login),
            _ => {
                self.status = "no token — press t to set a GitHub token".to_string();
                return;
            }
        };
        self.loading = true;
        self.error = None;
        self.status = format!("refreshing @{login} …");
        cx.notify();
        let this = cx.entity();
        cx.spawn(async move |_, cx| {
            ...
            this.update(cx, |inbox: &mut Inbox, cx| {
                ...
                match result {
                    Ok(response) => {
                        inbox.items = response
                            .items
                            .into_iter()
                            .filter(github::is_external)
                            .collect();
                        let fresh = inbox.items.iter().filter(|i| inbox.is_fresh(i)).count();
                        inbox.selected = 0;
                        inbox.error = None;
                        inbox.status = format!(
                    ...
                    Err(message) => {
                        ...
                        inbox.status = format!("refresh failed: {message}");
                    }
                }
                cx.notify();
                // Pre-select the first row so the panes are populated.
                if !inbox.visible().is_empty() {
                    inbox.do_select(0, cx);
                }
            });
        })
        .detach();
    ...
    pub fn do_select(&mut self, position: usize, cx: &mut Context<Self>) {
        let visible = self.visible();
        if visible.is_empty() {
            return;
        }
        self.selected = position.min(visible.len() - 1);
        if let Some(item) = visible
            .get(self.selected)
            .and_then(|index| self.items.get(*index))
        {
            self.seen
    ...
    pub fn move_selection(&mut self, delta: isize, cx: &mut Context<Self>) {
        ...
        if next != self.selected {
            self.do_select(next, cx);
        }
    }

    fn load_detail(&mut self, cx: &mut Context<Self>) {
        let (token, item) = match (self.token.clone(), self.selected_item()) {
            (Some(token), Some(item)) => (token, item),
            _ => return,
        };
    ...
    pub fn do_comment(&mut self, cx: &mut Context<Self>) {
        let (token, item) = match (self.token.clone(), self.selected_item()) {
            (Some(token), Some(item)) => (token, item),
            _ => {
        ...
        let this = cx.entity();
        overlay::open(
            &format!("comment on {}#{}", item.repo_full_name(), item.number),
            true,
            "",
            Box::new(move |value| {
                    ...
                    .spawn(async move |cx| {
                        ...
                        this.update(cx, |inbox: &mut Inbox, cx| {
                            match posted {
                                Ok(()) => {
                                    inbox.status =
                                        format!("comment posted on #{}", item.number);
                                    inbox.details.remove(&id);
                                    inbox.load_detail(cx);
                                }
                                Err(message) => {
    ...
    pub fn do_close(&mut self, cx: &mut Context<Self>) {
        let (token, item) = match (self.token.clone(), self.selected_item()) {
            (Some(token), Some(item)) => (token, item),
            _ => {
                self.status = "nothing to close".to_string();
                cx.notify();
                return;
            }
        };
        if !item.is_open() {
            self.status = "already closed — x only works on open items".to_string();
            cx.notify();
            return;
        }
        self.status = format!("closing #{} …", item.number);
        cx.notify();
        let this = cx.entity();
        cx.spawn(async move |_, cx| {
    ...
    pub fn do_token(&mut self, cx: &mut Context<Self>) {
        let async_cx = cx.to_async();
        let this = cx.entity();
        let initial = self.token.clone().unwrap_or_default();
        overlay::open(
            ...
            Box::new(move |value| {
                ...
                async_cx
                    .spawn(async move |cx| {
                        let login = github::get_login(&token).await;
                        this.update(cx, |inbox: &mut Inbox, cx| match login {
                            Ok(login) => {
...
fn is_stale(item: &Item) -> bool {
    ...
    let cutoff = OffsetDateTime::now_utc() - time::Duration::days(STALE_DAYS);
    updated < cutoff
}
            ...
            .map(|filter| {
                ...
                div()
                    ...
                    .on_click(move |_, _, cx| {
                        handler.update(cx, |inbox, cx| inbox.set_filter(filter, cx));
                    })
            })
            .collect::<Vec<_>>();
        ...
        let rows = visible
            ...
            .map(|(position, index)| {
                ...
                div()
                    ...
                    .on_click(move |_, _, cx| {
                        handler.update(cx, |inbox, cx| inbox.do_select(position, cx));
                    })
            })
            .collect::<Vec<_>>();
            ...
            .child(
                div()
                    ...
                    .child(
                        div()
                            .text_color(rgb(DIM))
                            .child("j/k move · enter open · r refresh · c comment · x close · t token · 1-4 filter"),
                    )
                    .child(
                        div()
                            .flex_1()
...
fn render_meta(selected: &Option<Item>, detail: &Option<Detail>) -> Vec<gpui::AnyElement> {
    ...
    let mut rows: Vec<gpui::AnyElement> = vec![
        ...
        div()
            ...
            .text_xs()
            .child(
                div().mr(px(8.0)).text_color(rgb(color)).child(format!("● {}", state)),
            )
            .child(
                div()
                    .text_color(rgb(DIM))
                ...
                div()
                    ...
                    .child(
                        div()
                            ...
                            .child(div().text_color(rgb(DIM)).child(format!("@{login} {when}"))),
                    )
                    .children(
                        review
                            .body
...
/// Wire global key bindings + actions. Call once after the window opens.
pub fn bind_keys(inbox: &Entity<Inbox>, cx: &mut App) {
    ...
    let check = move |_cx: &mut App| -> Option<Entity<Inbox>> {
        if overlay::is_open() {
            return None;
        }
        Some(guard.clone())
    ...
    cx.on_action({
        ...
        move |_: &OpenGithub, cx: &mut App| {
            if let Some(inbox) = check(cx) {
                inbox.read(cx).do_open();
            }
        }
    });
    cx.on_action({
        ...
        move |_: &Refresh, cx: &mut App| {
            if let Some(inbox) = check(cx) {
                inbox.update(cx, |inbox, cx| inbox.do_refresh(cx));
            }
        }
    });
    cx.on_action({
        ...
        move |_: &AddComment, cx: &mut App| {
            if let Some(inbox) = check(cx) {
                inbox.update(cx, |inbox, cx| inbox.do_comment(cx));
            }
        }
    });
    cx.on_action({
        ...
        move |_: &CloseItem, cx: &mut App| {
            if let Some(inbox) = check(cx) {
                inbox.update(cx, |inbox, cx| inbox.do_close(cx));
            }
        }
    });
    cx.on_action({
        ...
        move |_: &EditToken, cx: &mut App| {
            inbox.update(cx, |inbox, cx| inbox.do_token(cx));
        }
    });
    cx.on_action({
        let check = check.clone();
        move |_: &MoveDown, cx: &mut App| {
            if let Some(inbox) = check(cx) {
                inbox.update(cx, |inbox, cx| inbox.move_selection(1, cx));
            }
        }
    });
    cx.on_action({
    ...
    cx.on_action({
        ...
        move |_: &ShowOpen, cx: &mut App| {
            if let Some(inbox) = check(cx) {
                inbox.update(cx, |inbox, cx| inbox.set_filter(Filter::Open, cx));
            }
        }
    });
    cx.on_action({
        ...
        move |_: &ShowMerged, cx: &mut App| {
            if let Some(inbox) = check(cx) {
                inbox.update(cx, |inbox, cx| inbox.set_filter(Filter::Merged, cx));
            }
        }
    });
    cx.on_action({
        ...
        move |_: &ShowClosed, cx: &mut App| {
            if let Some(inbox) = check(cx) {
                inbox.update(cx, |inbox, cx| inbox.set_filter(Filter::Closed, cx));
            }
        }
    });
    cx.on_action({
        ...
        move |_: &ShowStale, cx: &mut App| {
            if let Some(inbox) = check(cx) {
                inbox.update(cx, |inbox, cx| inbox.set_filter(Filter::Stale, cx));
            }
        }
    });
}
