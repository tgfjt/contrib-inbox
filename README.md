...
## Sign in (OAuth, no tokens typed)

GitHub's OAuth endpoints send no CORS headers, so the PWA cannot call them
directly — `trunk serve` relays same-origin `/gh-oauth/*` → `https://github.com/*`
(see `[[proxy]]` in `Trunk.toml`).

One-time setup (2 min):

1. github.com → Settings → Developer settings → OAuth Apps → New OAuth App.
   Homepage URL can be anything local; add an **Authorization callback URL**
   exactly matching your serve origin, e.g.
   `http://127.0.0.1:8080/auth/callback` (use `127.0.0.1`, not `localhost`;
...
fallback link in the sign-in panel.
